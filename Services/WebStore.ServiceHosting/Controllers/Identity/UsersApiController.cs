...
namespace WebStore.ServiceHosting.Controllers.Identity
{
	...
	public class UsersApiController : ControllerBase
	{
		...
		{
			_logger = logger;
			_userStore = new UserStore<User, Role, WebStoreDB>(webStoreDB);
		}
        ...
        public async Task<string> SetNormalizedUserNameAsync([FromBody] User user, string name)
        {
            try
            {
                ...
                await _userStore.UpdateAsync(user);
                ($"User {user.UserName} normalize name seted as {name}").LogInfo(_logger);
        ...
        public async Task<bool> CreateAsync([FromBody] User user)
        {
			try
            {
                ...
                ($"User {user.UserName} created").LogInfo(_logger);
        ...
        public async Task<bool> RemoveClaimsAsync([FromBody] RemoveClaimDTO ClaimInfo, [FromServices] WebStoreDB db)
        {
			try 
            { 			
